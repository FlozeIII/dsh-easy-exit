...
window.__ModuleLoader__.load({
  ...
  factory(require) {
    ...
    /**
     * Whether the host accepted the shutdown.
     *
     * A dropped connection counts as acceptance: the tree disposes as it
     * answers, so there may be no reply at all. An answered 409 does NOT — that
     * is the running-job refusal, and treating it as acceptance closed the tab
     * for a server that was still running.
     *
     * @param response - the reply, or undefined when the request failed.
     * @returns True when the server is stopping.
     */
    async function shutdownAccepted(response) {
    ...
    function ExitButton(props) {
      ...
      const onClick = React.useCallback(async () => {
        ...
        if (phase === 'armed' || phase === 'refused') {
          ...
          if (await shutdownAccepted(reply)) {
      ...
      const label = stopped
        ...
        : refused
          ? tr('refused', 'Still running. Stopping would kill that work — click to try again.')
          : stopping
            ...
            ctx.effect(() => commandUi.register({
              ...
              ui: {
                ...
                onSelect: async () => {
                  ...
                  if (await shutdownAccepted(reply)) tryCloseTab()
                },
              },
