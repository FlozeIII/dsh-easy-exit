      ...
      return function ActionButton(props) {
        ...
        const onClick = React.useCallback(async () => {
          ...
          if (phase === 'armed' || phase === 'refused') {
            ...
            if (answer.accepted) {
              setPhase('stopped')
              // A restart keeps the tab: the launcher starts the server again and
              // the page reconnects on its own. Only a full stop closes it.
              if (restart) return
        ...
        const label = stopped
          ? tr(word('Stopped'), restart ? 'Restarting - the page will reconnect.' : 'The server has stopped. This page can be closed.')
          : refused
