...
/**
 * Exit code that asks the launcher to start the server again.
 *
 * 75 is `EX_TEMPFAIL` — "temporary failure, try again" — which is what a restart
 * is, and it is outside the range a normal exit or a shell's own failure codes
 * use here. Measured: `ctx.appExit(code)` really does reach the caller as the
 * process exit code, so the launcher can branch on it. A launcher that does not
 * know the code simply treats it as a non-zero exit and stops the server, which
 * The launcher sets `DSH_EASY_EXIT_RESTART_CODE`, so the convention lives in one
 * place: a launcher that prefers a different code can say so without the plugin
 * changing.
 */
function readRestartCode() {
  const declared = Number(process.env.DSH_EASY_EXIT_RESTART_CODE)
  return Number.isSafeInteger(declared) && declared > 0 && declared < 256 ? declared : 75
}

export const RESTART_EXIT_CODE = readRestartCode()
 ...
 *
 * `restart` leaves through {@link RESTART_EXIT_CODE} instead of 0. The plugin
 * never spawns a replacement process itself — the launcher owns that, which is
 * what keeps the port handover free of a race.
 *
 * @param ctx - the plugin context, consulted for jobs and the exit request.
 * @param options - `force` to override the guard, `restart` to ask for a relaunch.
 * @returns A machine-readable route plus the human-readable account to report.
 */
export function requestExit(ctx, options = {}) {
  const force = options?.force === true
  const restart = options?.restart === true
  const code = restart ? RESTART_EXIT_CODE : force ? 1 : 0
  ...
  if (busy.length > 0 && !force) {
    ...
    return {
      ...
      message:
        ...
        `Restarting or stopping the server now would kill that work. Wait for it to finish, or ask again with force ` +
        `to ${restart ? 'restart' : 'stop'} anyway.`,
    }
  }
  ...
  const restarted = restart
    ? 'Restart requested through the launcher; the server will stop and the launcher will start it again within a few seconds.'
    : undefined

  if (typeof appExit === 'function') {
    return {
      route: restart ? 'restart' : 'appExit',
      message:
        restarted ??
        (force
  ...
  return {
    route: 'signal',
    message: restart
      ? 'The launcher exit request was not available on this context, so a SIGINT is being sent instead. ' +
        'A SIGINT cannot carry the restart request: the launcher will stop the server rather than start it again.'
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      restart: {
        type: 'boolean',
        description:
          'Start the server again after stopping it, instead of leaving it stopped. The launcher performs the ' +
          'relaunch, so this only works when the server was started by the desktop launcher. Defaults to false.',
      },
      force: {
        type: 'boolean',
        description:
          'Stop or restart even though jobs are still running. Defaults to false; never set it without the user ' +
          'accepting that running work is lost.',
      },
    },
    output: {
    ...
    execute: async (args) => {
      const result = requestExit(ctx, { force: args?.force === true, restart: args?.restart === true })
      return {
  ...
  ctx.inject(['webServer', 'webRuntime'], webCtx => {
    webCtx.effect(() => webCtx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        let force = false
        // Restart travels in the query string. It is a single flag and the
        // request URL is always readable, whereas the body-borne flag was
        // measured to be ignored by the live web carrier even though the body
        // itself arrived intact. `force` stays on the body: it rides with the
        // browser's JSON call, and a carrier without a readable body simply
        // cannot escalate.
        let restart = false
        try {
          restart = /[?&]restart=(1|true)\b/.test(String(req.url ?? ''))
        } catch {
          restart = false
        }
        ...
        try {
          ...
          if (body.trim().length > 0) {
            ...
            if (parsed?.restart === true) restart = true
          }
        } catch {
          force = false
        }

        const result = requestExit(ctx, { force, restart })
        ...
        writeJson(res, result.route === 'duplicate' || result.route === 'blocked' ? 409 : 200, {
